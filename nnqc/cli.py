...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_dl = sub.add_parser(
        "download",
        help="download trained weights from Zenodo",
        epilog="The Zenodo record id is read from --record, the "
               "NNQC_ZENODO_RECORD_<TASK> environment variable, or the built-in "
               "ZENODO_RECORDS map in nnqc/hub.py, in that order.",
    )
    p_dl.add_argument("task", help="task name (e.g. prostate)")
    p_dl.add_argument("--record", default=None,
                      help="Zenodo record id (else NNQC_ZENODO_RECORD_<TASK> env)")
...
def main(argv=None) -> int:
    ...
    if args.command == "download":
        ...
        dest = download_weights(args.task, record_id=args.record, dest=args.dest,
