...
from open_clip import HFTokenizer, create_model_and_transforms, get_mean_std
...
class CrossAttentionGrid(nn.Module):
    ...
    def __init__(self,
        ...
        # Mirrored on the parent so build_context can apply the same projection.
        self.needs_proj_i = self.use_projection
...
class CLIPCrossAttentionGrid(nn.Module):
    ...
    def __init__(self,
        ...
        # Candidate-state gate (opt-in). Embeds the candidate mask's foreground
        # fraction as a third context token, so the UNet's cross-attention can
        # learn to lean on the scan token when the candidate is blank or thin -
        # today an empty candidate reconstructs to (near) nothing because the
        # only image signal is one CLIP token the model has no reason to trust
        # over an empty conditioning channel. Constructed ONLY when enabled so
        # the default state_dict is unchanged and existing xa.pt checkpoints
        # keep loading strictly. Enable via diffusion_train.xa_mask_gate.
        if mask_gate:
            self.mask_state = nn.Sequential(
                nn.Linear(1, 32), nn.GELU(), nn.Linear(32, output_dim))

        # The backbone path used to be a bare relative path, which broke as soon
        # as the process ran from anywhere but the repo root (as batch jobs do).
        # Resolve $NNQC_CLIP_PATH first, then the historical relative default.
        if clip_model_path is None:
            clip_model_path = os.environ.get(
                'NNQC_CLIP_PATH', 'trained_weights/unimed_clip_vit_b16.pt')
        if not os.path.isfile(clip_model_path):
            raise FileNotFoundError(
                f"UniMedCLIP backbone not found at {clip_model_path!r}. Download it with:\n"
                "  huggingface-cli download UzairK/unimed-clip-vit-b16 unimed-clip-vit-b16.pt\n"
                "then point $NNQC_CLIP_PATH at the .pt file."
            )
    ...
    def build_context(self, image, ext_features, mask=None):
        """Conditioning sequence for the diffusion UNet: ``[B, 2, output_dim]``
        (``[B, 3, output_dim]`` when the mask-state gate is enabled).

        Token 0 is the scan (UniMedCLIP), token 1 is the slice position; with
        the gate, token 2 embeds the candidate's foreground fraction. ``mask``
        is accepted and ignored when the gate is off, so call sites can pass it
        unconditionally.

        WHY NOT `forward(..., ext_features=...)`: that path routes both through
        ``CrossAttentionGrid``, which was written for N image patches against M
        text tokens. `encode_image_or_mask` returns CLIP's *pooled* embedding, so
        N=1, and the slice embedding gives M=1. Its `column_softmax` then takes
        ``softmax(dim=1)`` over a dimension of size 1, which is identically 1.0,
        and the result collapses to

            attended = attention_weights.T @ I_proj == I_proj

        i.e. **exactly independent of ext_features**. The slice-ratio embedding
        was silently discarded and its MLP received zero gradient, so the model
        had no position signal and could only reproduce the average cross-section
        - visible as apex/base slices coming out mid-sized. See
        ``tests/test_conditioning.py``.

        Emitting two tokens instead lets `DiffusionModelUNet` cross-attend to the
        scan and the position separately, which is what the architecture wanted.
        """
        image_features, _ = self.encode_image_or_mask(image, is_mask=False)  # [B, 1, D]
        if self.cross_attention.use_projection:
            image_features = self.cross_attention.proj_i(image_features)
        scan_token = self.cross_attention.output_proj(image_features.squeeze(1))

        if ext_features.dim() == 2:
            ext_features = ext_features.unsqueeze(1)
        pos_token = ext_features.squeeze(1).to(scan_token.device, scan_token.dtype)
        if pos_token.shape[-1] != scan_token.shape[-1]:
            raise ValueError(
                f"slice embedding is {pos_token.shape[-1]}-d but the context is "
                f"{scan_token.shape[-1]}-d; they must match "
                "(embed's output must equal cross_attention_dim)")

        gate = getattr(self, "mask_state", None)
        if gate is None:
            return torch.stack([scan_token, pos_token], dim=1)               # [B, 2, D]
        if mask is None:
            raise ValueError(
                "this XA was trained with the mask-state gate; build_context "
                "requires the candidate mask (mask=...)")
        frac = (mask > 0).float().mean(dim=(1, 2, 3)).unsqueeze(1)
        state_token = gate(frac.to(scan_token.device, scan_token.dtype))
        return torch.stack([scan_token, pos_token, state_token], dim=1)      # [B, 3, D]
